    ...
    def forward(self, x):
        ...
        for i in range(x.shape[0]):
            for v in product([-1.0, 0.0, 1.0], repeat=2):
                v = torch.tensor(v)
                xl.append(x[i, :] + v)
        X = torch.cat(xl).reshape(9 * self.n, 2)  # 9 экзмепляров каждой точки со сдвигами 

        # print(X)
        x2 = torch.square(X)
        x2s = torch.sum(x2, 1)
        distm = - 2 * X.mm(X.t()) + x2s + x2s.reshape((self.n * 9, 1)) + torch.eye(9 * self.n) * sqrt2
        # print(distm.shape)
        # print(self.mask.shape)
        dist_torus = torch.min(distm.unfold(0, 9, 9).unfold(1, 9, 9).reshape((self.n, self.n, 81)), 2).values
        # print(dist_torus)
        # diags = torch.sqrt(torch.masked_select(dist_torus,self.mask))
        diags = torch.masked_select(dist_torus, self.mask)
        # print(diags)
        # dist_lines = torch.abs(torch.masked_select(self.line_U.mm(x.t()) + self.line_v, self.mask_lines))   # не самый экономный способ, вычисляется много лишних расстояний
        # self.err = torch.max(dist_lines)
        return torch.max(
            diags)  # + penalty_coef * self.err    # второе слагаемое - "штрафная" функция; можно обойтись без нее, если задавать точки на ребрах по формуле p = a + \alpha(b-a)


class OptPartitionTorus():  # поиск оптимального разбиения, мультистарт

    def __init__(self, d, n, n_iter_circ=1000, n_iter_part=5000, lr_start=0.005, lr_decay=0.93, prec_opt=1 / 1000,
                 diam_tol=0.99, messages=1):
    ...
    def random_packing_torus(self):  # упаковка кругов в тор
        ...
        for i in range(self.n_iter1):
            ...
            if (i + 1) % messages_iter == 0:
                ...
                if self.messages >= 2:
                    print(i + 1, ' min_L = ', torch.min(dist_points), '   r = ', abs(float(y.detach().numpy())))
...
def find_true_diam(pts, polygons):
    ...
    for p in polygons:
        ...
        for i in range(len(p)):
            if p is None: return float('inf')
            if p[i] is None: return float('inf')
            if pts[p[i]] is None: return float('inf')
...
def plot_partition(part, find_true_diam=True, plot=True):
    ...
    if plot:
        ...
        plt.title(f" n: {part.n}   diam: {part.best_d:.10f}   true_diam: {true_diam:.10f}", fontsize=16)
        plt.show()
    return true_diam
...
if __name__ == "__main__":
    ...
    for n in range(5, 100):
        parts[n] = OptPartitionTorus(2, n)
    n = 26
    parts[n].multiple_runs(10)
